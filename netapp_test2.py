...
def case_type_check(lines):
    lines2 = lines.splitlines()
    for line in lines2:
        if 'CIVIL APPELLATE JURISDICTION' in line:
            return 'Civil'
        elif 'CRIMINAL APPELLATE JURISDICTION' in line:
            return 'Criminal'
    return None

def retrieve_case_type(filename):
    start = 'Prior_Cases/'
    try:
        with open(start + filename + '.txt', 'r') as f:
            lines = f.read()
            case_type = case_type_check(lines)
